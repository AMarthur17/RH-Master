...
const validarUsuario = async (usuarioId) => {
  const result = await db.query("SELECT id, nome, empresa FROM usuario WHERE id = $1", [usuarioId]);
...
router.get("/:usuario_id/relatorio", autenticar, permitir(["admin", "administrador"]), async (req, res) => {
  try {
    ...
    res.status(status).json({ error: error.message || "Erro ao gerar relatório" });
  }
});
...
router.get("/:usuario_id", autenticar, async (req, res) => {
  try {
    ...
    res.status(status).json({ error: error.message || "Erro ao listar benefícios" });
  }
});

router.post("/:usuario_id", autenticar, permitir(["admin", "administrador"]), async (req, res) => {
  try {
    const usuarioId = Number(req.params.usuario_id);
    ...
    if (!tipo || tipo.trim() === "") {
      return res.status(400).json({ error: "Tipo do benefício é obrigatório" });
    }
    ...
    res.status(201).json({ beneficio: { ...result.rows[0], valor: Number(result.rows[0].valor) } });
  } catch (error) {
    const status = error.statusCode || 500;
    console.error("[BENEFICIOS][CRIAR] Erro:", error);
    res.status(status).json({ error: error.message || "Erro ao criar benefício" });
  }
});
...
router.put("/:id", autenticar, permitir(["admin", "administrador"]), async (req, res) => {
  try {
    ...
    const beneficioAtual = await db.query("SELECT * FROM beneficios WHERE id = $1", [beneficioId]);
    ...
    if (tipo !== undefined) {
      if (tipo.trim() === "") {
        return res.status(400).json({ error: "Tipo do benefício não pode ser vazio" });
      }
    ...
    res.json({ beneficio: { ...result.rows[0], valor: Number(result.rows[0].valor) } });
  } catch (error) {
    const status = error.statusCode || 500;
    console.error("[BENEFICIOS][ATUALIZAR] Erro:", error);
    res.status(status).json({ error: error.message || "Erro ao atualizar benefício" });
  }
});
...
router.delete("/:id", autenticar, permitir(["admin", "administrador"]), async (req, res) => {
  try {
    ...
    const result = await db.query("DELETE FROM beneficios WHERE id = $1 RETURNING id", [beneficioId]);
    if (result.rows.length === 0) {
      return res.status(404).json({ error: "Benefício não encontrado" });
    }
    ...
    res.status(status).json({ error: error.message || "Erro ao remover benefício" });
  }
});
