...
export const verificarPermissao = (acao) => {
  return async (req, res, next) => {
    try {
      ...
      const documentoId = Number(req.params.documento_id || req.body.documento_id);

      // LOG para depuração
      console.log("[PERMISSAO] usuarioId:", usuarioId, "documentoId:", documentoId, "acao:", acao, "user:", req.user);

      if (Number.isNaN(documentoId) || !documentoId) {
        return res.status(400).json({ error: "ID do documento inválido" });
      }

      // Aceita tanto role quanto perfil para admin
      ...
      if (!permitido) {
        console.warn(`[PERMISSAO NEGADA] Usuário ${usuarioId} tentou ${acao} no documento ${documentoId}`);
